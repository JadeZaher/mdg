...
## 0.3.4 — docs push + LongMemEval headline

### Docs

- **Canonical CLI flag pass** across all docs and reference skills: `--clip` (not `--clip-chars`, which never existed), `--mp-stash-tag` (not `--mp-tag`, which still works as an alias). `mpg --help` is the single source of truth. Same pass applied to the bundled `skills/mpg-context/SKILL.md` and `skills/mpg-context/references/*.md`.
- **README headline rewritten** around the palace-adapter result; flat-log granite framing dropped in favour of neutral / framework-led numbers. New "Long-horizon memory" row replaces the old multi-turn row and points at the LongMemEval bench below.
...
- INSTALL.md: `mpg --version` example genericized to `0.3.x` so it won't drift on patch bumps; RPC param naming list corrected (`clip` / `mp_stash_tag` instead of `clip_chars` / `mp_tag`).

### Bench

- **`bench/longmemeval/`** added — the post-hoc analysis surface (`show_episode.py`, `analyze_misses.py`, `regrade.py`) from the LongMemEval run that produced the new headline number, plus a methodology README documenting which scripts are standalone vs harness-dependent. The result is auditable from a fresh clone of mpg alone, without needing the sibling benchmarks repo.
