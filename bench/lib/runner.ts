/**
 * Shared benchmark runner: invokes the built `mdg` CLI, parses JSON
 ...
 * measure when they run mdg from an agent.
 */
...
export function runMdg<T = unknown>(opts: RunOpts): RunResult<T> {
  const t0 = Date.now();
...
export function search(args: string[], opts?: Omit<RunOpts, "args">): RunResult<SearchJson> {
  return runMdg<SearchJson>({ ...opts, args: [...args, "--format", "json", "--no-color"] });
}
